...
pub const PacketType = enum {
    ...
    /// Optimization: Inline function with fast-path for data-plane packets (type 4)
    /// to improve branch prediction and avoid jump-table overhead.
    pub inline fn classify(data: []const u8) PacketType {
        ...
        // Fast-path: ~99% of packets are transport data
